...
public class ImageUtil {
	...
	public static BufferedImage thumb(BufferedImage source, int width,
		...
		logger.debug("生成缩略图 sourcce:{}width:{}height:{}b:{}",source,width,height,b);
	...
	public static void waterMark(String imgPath, String markPath, int x, int y,
		...
		try {
			...
			// 加载水印图片文件
			Image srcBiao = ImageIO.read(new File(markPath));
			g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP,
					alpha));
			g.drawImage(srcBiao, x, y, null);
			g.dispose();
	 ...
	 * @return BufferedImage 图片对象
	 */
	public static BufferedImage readJPEGImage(InputStream is)
	{
		try {
			// 得到输入的编码器，将文件流进行jpg格式编码
	...
	public static BufferedImage readPNGImage(File file)
	{
		try {
	...
	public static BufferedImage readPNGImage(InputStream is)
	{
		try {
	...
	public final static void cutImage(String srcImageFile,String result,  
			int strartW,int startH, int destWidth, int destHeight) {  
	    try {  
	        ...
	        Rectangle rectangle = new Rectangle(strartW,startH,destWidth,destHeight);/*指定截取范围*/      
	...
	public static void base64ToFile(String base64,String filepath,String fileName) throws IOException{
